...
**提供支持切换 Mihomo 核心的 Clash for Windows 前汉化版**
...
* 彻底逆向原版的混淆代码以允许审计, 贡献, 扩展
* 可选择使用 2023 年的旧 Clash 核心 或 使用支持 AnyTLS 等新协议的 Mihomo 核心 (在设置页面中切换)
* 维护频率会按照最低可用限度进行更新, 在我能力范围内提供不定期维护支持
...
*简单来说，这个库提供的Clash for Windows是修改过的，~请在向原版Clash for Windows反馈漏洞前先更换回原版~(原版已删库, 有问题可尝试在此提交 issue 但不保证修复)*

**修改的方式/内容大致说明列表**

    请查看过往提交历史, 现在提供的 Optimize 版本的代码库改动极大

代理核心支持：

* 默认使用 Mihomo 的 Core，
* 可在“设置 → 通用 → 代理核心”切换为 2023 Clash Core；切换后应用会自动重启代理核心以兼容旧的配置和使用习惯。
* Mihomo 模式支持 AnyTLS。内置配置编辑器可编辑常用 AnyTLS 字段，并在再次编辑时保留 ShadowTLS、ResTLS、JLS 等高级字段。
* 当前构建资源包含 Windows x64、Windows x86、Windows ARM64、Linux x64、Linux ARM64、macOS Intel 和 macOS Apple Silicon 的 Mihomo 核心。
...
开发前请阅读 [`AGENTS.md`](AGENTS.md) 与 [`docs/`](docs/README.md)。统一测试入口为 `npm test`，可在 Windows、Linux 和 macOS 上运行。
